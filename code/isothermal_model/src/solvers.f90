...
100     continue
    ...
    function init_euler_implicit_config() result(config)
        ...
        config%dt_step = dt_step_impl   ! default 10 ms; overridable via
        ! &solver namelist (dt_step_impl) or --dt-step=<seconds> CLI flag.
        ! A-stability lets implicit Euler take much larger steps than forward Euler.
        config%rtol = 1.0e-6_DP      ! Newton convergence tolerance on ||delta||
        config%mxstep = 100          ! max Newton iterations per step
