...
program prototype_reactor
    ...
    character(len=64)     :: solver_choice, dt_step_cli
    ...
    ! --- Parse CLI args -----------------------------------------------------
    call parse_args(solver_choice, config_path, dt_step_cli)
    ...
    ! --- CLI override wins over namelist for implicit-Euler dt -------------
    if (len_trim(dt_step_cli) > 0) then
        read (dt_step_cli, *, iostat=dt_step_ios) dt_step_impl
        if (dt_step_ios /= 0) then
            write (*, '(A,A)') '  ERROR: cannot parse --dt-step value: ', trim(dt_step_cli)
            stop 1
        end if
    end if
    ...
    call write_integrated_rates(run_dir, tt, yy)
...
contains
    ...
    ! --------------------------------------------------------------------------
    ! parse_args — read --solver=<name> (required), --config=<path> (optional),
    ! and --dt-step=<seconds> (optional, overrides &solver/dt_step_impl).
    ! Also handles -h / --help.
    ! --------------------------------------------------------------------------
    subroutine parse_args(solver, cfg_path, dt_step_str)
        character(len=*), intent(out) :: solver
        character(len=*), intent(out) :: cfg_path
        character(len=*), intent(out) :: dt_step_str
        integer :: k, nargs
        ...
        do k = 1, nargs
            ...
            else if (index(arg, '--dt-step=') == 1) then
                dt_step_str = value_after(arg, '--dt-step=')
